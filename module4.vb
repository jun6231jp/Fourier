...
Dim count As Long
Dim chk As Integer
MsgBox ("波の重ね合わせで三角波形を作ります。")

'設定値を入力
    delta = InputBox("三角波の幅" & vbCrLf & "" & vbCrLf & "波形幅に対し振幅が変化し" & vbCrLf & "面積1となる波形を生成します。" & vbCrLf & "例：0.5")
...
Else
    start = InputBox("θ最小値" & vbCrLf & "" & vbCrLf & "波形の横軸の左端です。" & vbCrLf & "2π付近の値を推奨します。" & vbCrLf & "例：5.5")
...
Wend
'フーリエ級数展開の表作成
mktbl4 delta, n, count
For i = 0 To n
   draw_data5 count, i, start, last, grid, delta
Next i
draw_data6 count, n, start, last, grid, delta
End If
End If
...
Sub mktbl4(delta As Double, n As Integer, k As Long) 'フーリエ級数展開の表の中身を作成
 Dim i As Integer
 Dim j As Integer
 For i = 1 To n
  For j = 1 To k
   Sheet2.Cells(2 + j, 2 + i).Value = Sheet2.Cells(2 + j, 1 + i).Value + (((1 - Cos(delta * i)) / (delta * i * delta * i)) * Cos(i * Sheet2.Cells(2 + j, 1))) * 2 / 3.14 + (((delta * i - Sin(delta * i)) / (delta * i * delta * i)) * Sin(i * Sheet2.Cells(2 + j, 1))) * 2 / 3.14
  Next j
 Next i
End Sub

Sub draw_data5(length As Long, n As Integer, start As Double, last As Double, grid As Double, delta As Double)    '外側から引数をFor文で与えることでアニメーションでグラフを表示
    Dim i As Long
    ...
    ActiveChart.ChartTitle.Characters.Text = "三角波生成の過程"   ' グラフタイトル
    ...
    ActiveChart.Axes(xlValue, xlPrimary).MinimumScale = -0.5 / delta  ' y 軸の最小値
    ActiveChart.Axes(xlValue, xlPrimary).MaximumScale = 4 / delta ' y 軸の最大値
    ActiveChart.Axes(xlValue, xlPrimary).MajorUnit = 0.5 / delta    ' y 軸の目盛幅
'For i = 0 To n
...
Sub draw_data6(length As Long, n As Integer, start As Double, last As Double, grid As Double, delta As Double)    '最後に全ての線を重ねて自動グラフ作成
    Dim i As Long
    Dim j As Long
    Dim x As Double, dx As Double
    For j = 0 To n
    ...
    ActiveChart.HasTitle = True
    ActiveChart.ChartTitle.Characters.Text = "三角波生成結果"   ' グラフタイトル
    ActiveChart.Axes(xlCategory, xlPrimary).HasTitle = True
    ActiveChart.Axes(xlCategory, xlPrimary).AxisTitle.Characters.Text = "θ"    ' x 軸タイトル
    ActiveChart.Axes(xlValue, xlPrimary).HasTitle = True
    ActiveChart.Axes(xlValue, xlPrimary).AxisTitle.Characters.Text = "P"       ' y 軸タイトル
